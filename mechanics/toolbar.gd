...
func _play_electrocute() -> void:
	...
	taser_impact.position = center + Vector2(-3250, -3000) - taser_impact.size / 2
	taser_impact.scale = Vector2(4, 4)
	taser_impact.modulate.a = 1.0
	taser_impact.visible = true

	electric_wire_impact.position = center + Vector2(-3250, -3000) - electric_wire_impact.size / 2
...
func _play_fry() -> void:
	var center := _get_ceiling_screen_pos()
	print(center)
	var pan_target_pos := center + Vector2(-1750, -1500) - frying_impact.size / 2
